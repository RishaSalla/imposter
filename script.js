...
function renderPlayerList() {
  ...
  gameState.players.forEach((player, index) => {
    ...
    playerEl.innerHTML = `
      ...
      <button data-index="${index}" class="remove-player-btn text-red-500 hover:text-red-700 p-1 rounded-full transition-colors" aria-label="حذف اللاعب">
...
function updateStartButtonState() {
  ...
  if (isReady) {
    elements.startGameBtn.classList.remove('bg-red-400', 'opacity-70', 'cursor-not-allowed');
    elements.startGameBtn.classList.add('bg-red-600', 'hover:bg-red-700');
  } else {
    elements.startGameBtn.classList.add('bg-red-400', 'opacity-70', 'cursor-not-allowed');
    elements.startGameBtn.classList.remove('bg-red-600', 'hover:bg-red-700');
  }
}
...
function showPassScreen() {
  ...
  elements.passPrompt.textContent = (gameState.currentPlayerIndex === 0)
    ? "مرّر الجهاز لأول لاعب!"
    : "مرّر الجهاز للاعب التالي!";
  switchScreen('pass');
}
...
function showSecretScreen() {
  ...
  if (gameState.mode === MODE.QUESTIONS) {
    if (isImposter) {
      elements.secretTitle.textContent = "أنت المحتال! (لا تكشف هويتك)";
      elements.secretQuestionText.textContent = gameState.currentSet.imposter;
    } else {
      elements.secretTitle.textContent = "أنت لاعب عادي. السؤال السري:";
      elements.secretQuestionText.textContent = gameState.currentSet.normal;
    }
    // Set colors for questions mode
    elements.secretTitle.classList.toggle('text-red-400', isImposter);
    ...
    if (isImposter) {
      elements.secretTitle.textContent = "أنت المحتال! (الكلمة السرية مجهولة لك)";
      elements.secretQuestionText.textContent = '— لا كلمة سر —';
    } else {
      elements.secretTitle.textContent = "أنت لاعب عادي. الكلمة السرية:";
      elements.secretQuestionText.textContent = gameState.currentSet.normal;
    }
...
function showSecretVoteScreen() {
  ...
  gameState.players.forEach((player, index) => {
    if (index !== gameState.currentVoterIndex) { // Can't vote for self
      ...
      voteButton.className = 'vote-btn px-4 py-3 text-lg font-bold text-gray-800 bg-white rounded-xl shadow-md transition hover:bg-red-500 hover:text-white focus:outline-none focus:ring-4 focus:ring-red-300';
...
function handleVote(targetIndex) {
  ...
  // Pass the device to the next voter (we reuse the pass screen logic for secrecy)
  const nextVoterName = gameState.players[gameState.currentVoterIndex];
  if (nextVoterName) {
    elements.passPlayerName.textContent = nextVoterName;
    elements.passPrompt.textContent = "مرّر الجهاز للتصويت سراً.";
    switchScreen('pass');
  } else {
    // All done voting
    showResultScreen();
...
function showResultScreen() {
  ...
  elements.imposterReveal.classList.add('text-red-600');
  ...
  if (imposterVotedOut) {
    ...
    winnerClass = 'bg-red-100 text-red-800';
  } else {
    // Should not happen, but for safety
    winnerText = `انتهت اللعبة بالتعادل.`;
    winnerClass = 'bg-gray-100 text-gray-800';
  }
...
// Function to handle temporary UI message on reset
function showResetConfirmation() {
  const originalText = elements.resetHistoryBtn.textContent;
  elements.resetHistoryBtn.textContent = '✅ تم إعادة التعيين!';
  elements.resetHistoryBtn.classList.add('text-green-600', 'font-bold', 'no-underline');
  elements.resetHistoryBtn.classList.remove('text-red-600', 'underline');

  setTimeout(() => {
    elements.resetHistoryBtn.textContent = originalText;
    elements.resetHistoryBtn.classList.add('text-red-600', 'underline');
    elements.resetHistoryBtn.classList.remove('text-green-600', 'font-bold', 'no-underline');
  }, 1500);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  elements.resetHistoryBtn.addEventListener('click', () => {
    localStorage.removeItem(STORAGE_KEYS.q);
    localStorage.removeItem(STORAGE_KEYS.w);
    showResetConfirmation(); // Use the new function instead of alert()
  });

  // Pass Screen Listeners
  elements.confirmPlayerBtn.addEventListener('click', () => {
    // Check if the current phase is card reveal or final voting reveal
    if (gameState.currentPlayerIndex < gameState.players.length) {
      showRevealPromptScreen();
    } else if (gameState.currentVoterIndex < gameState.players.length) {
      showSecretVoteScreen();
    }
  });
