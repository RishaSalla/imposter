...
function switchScreen(targetScreenId) {
  ...
  if (targetScreen) {
    ...
    updateStepIndicator(targetScreenId);
  }
}
...
function updateStepIndicator(screenId) {
  const stepMap = {
    'home': 1, 'setup': 1,
    'pass': 2, 'reveal-prompt': 2, 'secret': 2,
    'discuss': 3,
    'secret-vote': 4,
    'result': 5
  };
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (stepEl) {
      if (i === currentStep) {
        stepEl.classList.add('bg-white/70', 'font-bold');
        stepEl.classList.remove('bg-white/30');
      } else {
        stepEl.classList.add('bg-white/30');
        stepEl.classList.remove('bg-white/70', 'font-bold');
      }
    }
  }
}
...
function updateStartButtonState() {
  ...
  if (isReady) {
    elements.startGameBtn.classList.remove('bg-pink-400', 'opacity-70', 'cursor-not-allowed');
    elements.startGameBtn.classList.add('bg-pink-600', 'hover:bg-pink-700');
  } else {
    elements.startGameBtn.classList.add('bg-pink-400', 'opacity-70', 'cursor-not-allowed');
    elements.startGameBtn.classList.remove('bg-pink-600', 'hover:bg-pink-700');
  }
}
...
function showSecretScreen() {
  ...
  if (gameState.mode === MODE.QUESTIONS) {
    ...
    elements.secretTitle.classList.toggle('text-teal-400', !isImposter);
...
function showResultScreen() {
  ...
  elements.imposterReveal.classList.remove('text-teal-700');
  ...
  if (imposterVotedOut) {
    ...
    winnerClass = 'bg-teal-100 text-teal-800';
...
function showResetConfirmation() {
  ...
  elements.resetHistoryBtn.classList.add('text-teal-600', 'font-bold', 'no-underline');
  elements.resetHistoryBtn.classList.remove('text-slate-600', 'underline');

  setTimeout(() => {
    elements.resetHistoryBtn.textContent = originalText;
    elements.resetHistoryBtn.classList.add('text-slate-600', 'underline');
    elements.resetHistoryBtn.classList.remove('text-teal-600', 'font-bold', 'no-underline');
  }, 1500);
}
